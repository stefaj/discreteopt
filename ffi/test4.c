...
/*int callback (CPXCENVptr env,
            ...
            int *useraction_p)*/ 
    {
        ...
        if(x[0] > 20)
        {
          ...
          // status = CPXaddusercuts(env, lp, 1, 3, crhs, csense, cmatbeg, cmatind, cmatval, NULL );
            //status = CPXcutcallbackadd(env, cbdata, wherefrom, 3, 20, 'L', cmatind, cmatval, CPX_USECUT_FORCE);
        }
        //
        if ( status ) {
          fprintf (stderr, "Some error, status = %d.\n", status);
        }
 ...
 int main(int argc, char **argv)
 {
    ...
    status = CPXsetincumbentcallbackfunc (env, callback,
                                      &lp);
